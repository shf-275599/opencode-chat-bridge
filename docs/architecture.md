...
本文档旨在为开发者提供 `opencode-im-bridge` 的架构设计说明，涵盖模块布局、关键抽象、数据流以及系统扩展方法。
...
1. **接收**: 平台 Plugin 通过各自协议接收原始事件（WebSocket / HTTP 长轮询 / Bot API）
...
6. **Register Plugins**: 实例化并注册各渠道插件（如 `FeishuPlugin`, `WechatPlugin`）
7. **Start Channels**: 启动各渠道的连接（WebSocket / HTTP 长轮询 / Bot API）
